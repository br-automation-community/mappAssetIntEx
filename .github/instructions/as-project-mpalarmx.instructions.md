...
1. Verify XML structure by asking the B&R Automation Studio Help Server (via `mcp__as-help__*` tools) for the relevant file type and comparing with already existing mappAlarmX XMLs in the project.
...
Use the B&R Automation Studio Help Server (via `mcp__as-help__*` tools) for parameter-level confirmation:
