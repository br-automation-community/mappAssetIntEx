...
- **Documentation:** Always refer to the B&R Automation Studio Help Server (via `mcp__as-help__*` tools) for mappView documentation and best practices.
...
- The default units are based on the "UNECE/CEFACT Trade Facilitation Recommendation No.20" standard. Use `unit="{'metric':'CEL','imperial':'FAH','imperial-us':'KEL'}"`. Look for `Available standard units` in the B&R Automation Studio Help Server (via `mcp__as-help__*` tools).
...
**Available event sources** (not limited to OPC UA — consult the B&R Automation Studio Help Server via `mcp__as-help__*` tools for complete documentation on each):
...
Widget properties fall into two categories. Check the widget documentation in the B&R Automation Studio Help Server (via `mcp__as-help__*` tools) to determine which category a property belongs to.
...
- Find the full list of widgets in the B&R Automation Studio Help Server (via `mcp__as-help__*` tools). Consider performance implications of widget classes (A, B, C) relative to the target hardware (e.g., avoid Class C on T50 panels).
