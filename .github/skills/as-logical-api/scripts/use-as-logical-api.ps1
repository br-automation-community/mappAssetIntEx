<#
.SYNOPSIS
    Manage B&R Automation Studio Logical view: add libraries and ST program tasks.
...
.PARAMETER Action
    The action to perform: AddLibrary | AddSTTask
    Default: AddLibrary
...
.PARAMETER TaskPath
    Folder path relative to Logical/ where the task will be created (required when Action=AddSTTask).
    Examples: "Drive", "Service/SubFolder"

.PARAMETER TaskName
    Name of the program task to create, max 10 characters (required when Action=AddSTTask).
...
param(
    ...
    [Parameter()]
    [ValidateSet("AddLibrary", "AddSTTask")]
...
function Register-TaskInSwFile {
    ...
    $taskEntry     = "    <Task Name=`"$ShortName`" Source=`"$Source`" Memory=`"UserROM`" Language=`"IEC`" Debugging=`"true`"></Task>"
...
'@
    ...
    Register-TaskInSwFile -SwFilePath $swFilePath -ShortName $shortName -Source $source -TaskClassNum $TaskClass
